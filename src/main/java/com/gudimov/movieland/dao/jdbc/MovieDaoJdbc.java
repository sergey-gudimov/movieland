...
public class MovieDaoJdbc implements MovieDao {
    ...
    public List<MovieWithGenreCountry> getRandom() {
        ...
        List<Movie> listMovie = jdbcTemplate.query(getRandomMovieSQL, new MovieRowMapper());
        log.info("Finish query to get movie from DB");
        List<MovieWithGenreCountry> movieWithGenreCountries;
        movieWithGenreCountries=MovieEnricher.EnrichMovieWithGenre(listMovie);
        log.info("Finish enriched with genres");
        movieWithGenreCountries=MovieEnricher.EnrichMovieWithCountry(listMovie);
